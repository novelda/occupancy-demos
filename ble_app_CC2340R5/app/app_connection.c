...
void Connection_ConnEventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    switch(event)
    {
        case BLEAPPUTIL_LINK_ESTABLISHED_EVENT:
        {
            ...
            Display_printf(handle, 0, 0, "Established connection - Connected to %s connectionHandle = %d", BLEAppUtil_convertBdAddr2Str(gapEstMsg->devAddr), gapEstMsg->connectionHandle);

            /*! Print the number of current connections */
            Display_printf(handle, 0, 0, "Connections number: %d", linkDB_NumActive());
            OccupancyProfile_on_connect();
        ...
        case BLEAPPUTIL_LINK_TERMINATED_EVENT:
        {
            ...
            /*! Print the peer address and connection handle number */
            Display_printf(handle, 0, 0, "Connection terminated - connectionHandle = %d reason = %d", gapTermMsg->connectionHandle, gapTermMsg->reason);

            /*! Print the number of current connections */
            Display_printf(handle, 0, 0, "Connections number: %d", linkDB_NumActive());
            OccupancyProfile_on_disconnect();
        ...
        case BLEAPPUTIL_LINK_PARAM_UPDATE_EVENT:
        {
            ...
            if (linkDB_GetInfo(pPkt->connectionHandle, &linkInfo) ==  SUCCESS)
            {
              ...
              {
                  Display_printf(handle, 0, 0, "Parameter update - connectionHandle = %d", pPkt->connectionHandle);
              }
              else
              {
