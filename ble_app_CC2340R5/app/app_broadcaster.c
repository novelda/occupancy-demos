...
void Broadcaster_AdvEventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    ...
    switch(event)
    {
        ...
        {
            Display_printf(handle, 0, 0, "Started advertising - handle: %d", advData->pBuf->advHandle);
            break;
        }

        case BLEAPPUTIL_ADV_END_AFTER_DISABLE:
        {
            Display_printf(handle, 0, 0, "Advertising stopped - handle: %d", advData->pBuf->advHandle);
            break;
        }
