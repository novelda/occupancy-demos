...
void Observer_ScanEventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
    ...
    switch (event)
    {
        ...
        {
            Display_printf(handle, 0, 0, "Scan status: Adv report");
        ...
        {
            Display_printf(handle, 0, 0, "Scan status: Scan started...");
        ...
        case BLEAPPUTIL_SCAN_DISABLED:
        {
            ...
            Display_printf(handle, 0, 0, "Scan status: Scan disabled - Reason: %d Num results: %d", scanMsg->pBuf->pScanDis.reason, scanMsg->pBuf->pScanDis.numReport);
            break;
        }
