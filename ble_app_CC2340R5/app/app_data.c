...
static void GATT_EventHandler(uint32 event, BLEAppUtil_msgHdr_t *pMsgData)
{
  ...
  switch ( gattMsg->method )
  {
    case ATT_FLOW_CTRL_VIOLATED_EVENT:
      {
          Display_printf(handle, 0, 0, "GATT status: ATT flow control is violated");
      }
      break;

    case ATT_MTU_UPDATED_EVENT:
      {
          Display_printf(handle, 0, 0, "GATT status: ATT MTU update to %d", (( attMtuUpdatedEvt_t * )pMsgData)->MTU);
      }
      break;
